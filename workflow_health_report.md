...
**Aktualisiert:** 2026-05-30 05:00 UTC
...
| Datei | Check | Detail |
|---|---|---|
| `Production Health` | Aktualität: Confidence-40 Blacklist | blacklist_confidence40_ipv4.txt ist 24h alt (CRITICAL-Schwelle: 10h) – Pipeline-Stillstand |
| `Production Health` | active ⊆ conf40 Subset-Invariante verletzt | 29,326 IPs in active fehlen in conf40 (7.354% von active). Ursache vermutlich Cache-Drift zwischen combined- und confidence-Workflow (siehe BUG-CACHE-DRIFT). Der Heilungs-Pfad in update_confidence_blacklist.yml hat entweder nicht gegriffen (Cap >10%) oder wurde umgangen. |
...
| Level | Check | Detail |
|---|---|---|
| 🔴 CRITICAL | Aktualität: Confidence-40 Blacklist | blacklist_confidence40_ipv4.txt ist 24h alt (CRITICAL-Schwelle: 10h) – Pipeline-Stillstand |
| 🔴 CRITICAL | active ⊆ conf40 Subset-Invariante verletzt | 29,326 IPs in active fehlen in conf40 (7.354% von active). Ursache vermutlich Cache-Drift zwischen combined- und confidence-Workflow (siehe BUG-CACHE-DRIFT). Der Heilungs-Pfad in update_confidence_blacklist.yml hat entweder nicht gegriffen (Cap >10%) oder wurde umgangen. |
...
---
*Generiert: 2026-05-30 05:00 UTC | 23 Workflow-Dateien geprüft*