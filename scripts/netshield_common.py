...
# ───────────────────────────────────────────────────────────────
# SECURITY FIX 2026-08-31: Auth-/API-Header nicht ueber Origins leaken.
#
# urllib.request.HTTPRedirectHandler uebernimmt Request-Header bei Redirects.
# Das ist fuer harmlose Header okay, aber bei einem Redirect auf einen anderen
# Origin duerfen Authorization/API-Key/Cookie/Token-Header niemals mitwandern.
# Sonst koennte ein kompromittierter Feed/API-Endpunkt per 30x auf einen
# fremden Host umleiten und dort das Secret abgreifen.
# ───────────────────────────────────────────────────────────────
def _redirect_origin(url):
    """Normalisierte Origin (scheme, host, effektiver Port) fuer Redirect-Checks."""
    import urllib.parse
...
def _is_sensitive_redirect_header(name):
    """True fuer Header, die nie an einen fremden Redirect-Origin gehen duerfen."""
    ...
    if n in {
        ...
        return True
    return n.endswith(("-token", "-secret", "-key"))


def _strip_sensitive_headers_for_redirect(request, old_url, new_url):
    """Entfernt Secrets aus einem Redirect-Request bei Origin-Wechsel.

    Gleicher Origin (inkl. effektiv gleichem Default-Port) behaelt die Header,
    damit legitime Same-Origin-Redirects authentifizierter APIs funktionieren.
    Bei Host-, Port- oder Schemawechsel werden nur sensible Header entfernt;
    User-Agent/Accept usw. bleiben erhalten.
    """
    if _redirect_origin(old_url) == _redirect_origin(new_url):
        return request
    ...
    for attr in ("headers", "unredirected_hdrs"):
        ...
        if not mapping:
            continue
        for header_name in list(mapping.keys()):
            if _is_sensitive_redirect_header(header_name):
                mapping.pop(header_name, None)
    return request
        ...
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            ...
            if redirected is not None:
                _strip_sensitive_headers_for_redirect(
                    redirected, req.full_url, newurl)
            return redirected
...
class SqliteSeenDB(_MutableMapping):
    ...
    def _init_schema(self):
        ...
        self._conn.execute(
            ...
            "auto_today_count INTEGER)"
        )
        # Schema-Migration fuer bereits vorhandene persistente Snapshots.
        # ALTER TABLE ist idempotent ueber PRAGMA table_info abgesichert.
        _cols = {r[1] for r in self._conn.execute("PRAGMA table_info(seen_db)")}
        if "auto_today_count" not in _cols:
            self._conn.execute(
    ...
    def _row_to_dict(self, row):
        ...
        # transienter Kanal von auto_feed_discovery zu update_combined_blacklist
        # und existiert nur bei den wenigen IPs, die AFD in seinem letzten Lauf
        # angefasst hat. Wuerde der Key IMMER gesetzt, stuende in
        # seen_db.json bei ~9,5 Mio. Eintraegen ein "auto_today_count":null -
        # rund 24 Byte pro Eintrag, also gut 200 MB Zuwachs, plus zusaetzliche
        # gzip-Backup-Parts und Cache-Transfer. Der Key wird deshalb nur
        # ausgegeben, wenn er tatsaechlich belegt ist. Damit bleibt
        # export_json_atomic() formatgleich zu dem, was write_json_atomic()
        # vor der SQLite-Migration geschrieben hat. Leseseitig macht das
        # keinen Unterschied: update_combined_blacklist greift ueber
        # entry.pop("auto_today_count", None) mit Default darauf zu.
        if auto_today_count is not None:
            entry["auto_today_count"] = auto_today_count
        return entry
        ...
        for ip, entry in plain_dict.items():
            ...
            if len(_batch) >= batch_size:
                self._conn.executemany(
                    ...
                    "VALUES (" + ",".join("?" * len(self._COLUMNS)) + ")", _batch)
                self._conn.commit()
                _n += len(_batch)
                _batch = []
        if _batch:
